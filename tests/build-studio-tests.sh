...
# clang rejects -static-libgcc, so keep it MSYS/Cygwin-only. (Append —
# an empty array expansion breaks under set -u on macOS bash 3.2.)
case "$OSTYPE" in
    msys*|cygwin*|win32*) CXXFLAGS+=(-static-libstdc++ -static-libgcc) ;;
esac
