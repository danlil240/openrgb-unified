...
:: out\ — expected vs present, files must also be non-    ::
:: empty (a truncated 0-byte DLL is a miss), exit 1 on    ::
:: any failure. Read-                                     ::
...
:need_file
:: File entries must exist AND be non-empty — a truncated
:: 0-byte DLL passes `if exist` but fails to load.
set /a CHECKED+=1
if not exist "%~1" (echo   MISSING %~1 & set /a MISSING+=1 & exit /b 0)
if %~z1==0 (echo   EMPTY   %~1 & set /a MISSING+=1 & exit /b 0)
echo   ok      %~1
exit /b 0
